...
public interface PlaceRepository extends PagingAndSortingRepository<Place,Long>, JpaSpecificationExecutor<Place> {
    ...
    List<Place> findAllPlaceByDistanceAndPoint(Point<G2D> refPnt, double dist);


    @Query("SELECT vl, distance(vl.location, ?1) as distance from Place vl order by distance" )
    List<Tuple> findNearestPoint(Point<G2D> refPnt, Pageable page);


    @Query("SELECT vl FROM Place AS vl WHERE within(vl.location, :filter)=TRUE ")
    List<Place> findWithin(@Param("filter") Geometry filter);

    //search
    List<Place> findAll(Specification<Place> specification);
}